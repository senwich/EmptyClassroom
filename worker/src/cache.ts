...
export async function getCachedClassInfo(env: Env, key = TODAY_CACHE_KEY): Promise<ClassInfo | null> {
  const raw = await env.EC_CACHE.get(key);
...
export async function setCachedClassInfo(env: Env, classInfo: ClassInfo): Promise<void> {
  ...
  await Promise.all([
    env.EC_CACHE.put(TODAY_CACHE_KEY, raw, { expirationTtl: FRESH_TTL_SECONDS }),
    env.EC_CACHE.put(TODAY_STALE_CACHE_KEY, raw, { expirationTtl: STALE_TTL_SECONDS }),
  ]);
}
